...
//! the focused mode; this module adds the other half: when the main agent
//! opens a long goal wait window (declared via `update_goal(waiting_on)`),
//! a cheap READ-ONLY subagent is spawned in the background to wander over
//! the mission map, the blackboard, and the workspace, and drop at most a
//! few `idea` entries into the silent idea box.
//!
...
/// Per-session last-run stamps. A module-local static (keyed by session id)
/// instead of a `SessionActor` field: the actor is built by struct literals
/// at many sites and this state is fire-and-forget telemetry-grade.
...
pub(crate) fn should_incubate(session_id: &str, wait_secs: u64) -> bool {
