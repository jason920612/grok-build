...
pub(crate) async fn run_shell_child(
    ...
    // Upstream's resolver does role selection + override resolution +
    // definition defaults; it is fed the roster-augmented persona map so a
    // roster identity resolves exactly like a configured persona.
