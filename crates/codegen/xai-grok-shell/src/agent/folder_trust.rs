...
mod tests {
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; GROKTOOL_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because GROKTOOL_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. GROKTOOL_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because GROKTOOL_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROKTOOL_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. GROKTOOL_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because GROKTOOL_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROKTOOL_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; GROKTOOL_HOME-isolated store; GROK_FOLDER_TRUST unset so the
        // default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _home = EnvGuard::set("HOME", home.path());
        let grok_home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROKTOOL_HOME", grok_home.path());
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. GROKTOOL_HOME-isolated so the trust store is empty;
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. GROKTOOL_HOME-isolated so the trust store is empty;
        // GROK_FOLDER_TRUST unset so the default-on feature flag applies.
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). GROKTOOL_HOME-isolated so the trust store is empty.
        use xai_grok_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROKTOOL_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. GROKTOOL_HOME-isolated (empty store).
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // fail-closed rather than defaulting open). GROKTOOL_HOME-isolated (empty
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + GROKTOOL_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. GROKTOOL_HOME-isolated so the seeded store is the temp one;
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        ...
        // discover_plugins/build_for_cwd/reload. GROKTOOL_HOME-isolated (empty store);
        // GROK_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROKTOOL_HOME", home.path());
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // GROKTOOL_HOME-isolated (empty store); GROK_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROKTOOL_HOME", home.path());
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // GROKTOOL_HOME-isolated so both the folder-trust store and the plugin trust
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // GROKTOOL_HOME-isolated so the grant writes to a temp store; GROK_FOLDER_TRUST unset
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // GROKTOOL_HOME so the store is empty/seeded in temp. `#[serial]` because both
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // unset so `is_local_build()` is genuinely true. GROKTOOL_HOME-isolated so the
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. GROKTOOL_HOME-isolated so the store starts
        // empty; `#[serial]` because GROKTOOL_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROKTOOL_HOME", home.path());
        let tmp = repo_tmp();
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; GROKTOOL_HOME-isolated and
