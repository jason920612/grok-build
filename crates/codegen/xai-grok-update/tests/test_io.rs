...
//!
//! These tests touch global process state — `GROKTOOL_HOME` (a `OnceLock` in
//! `xai-grok-config`), `GROK_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `GROKTOOL_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the GROKTOOL_HOME isolation used in other
//! integration tests.
...
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under GROKTOOL_HOME,
