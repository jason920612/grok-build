...
/// The blocking-wait cap for this call: the standard [`max_wait_block`],
/// tightened to [`crate::guardrails::GOAL_WAIT_BLOCK_CAP`] while the goal
/// loop is active (guardrail `goal_block_cap`) so the agent cannot camp the
/// turn by re-waiting — it should declare `update_goal(waiting_on: ...)` and
/// let completion wake it.
pub(crate) fn goal_wait_cap(goal_active: bool) -> Duration {
    if goal_active && crate::guardrails::guardrails().goal_block_cap {
        max_wait_block().min(crate::guardrails::GOAL_WAIT_BLOCK_CAP)
    } else {
        max_wait_block()
    }
}

/// Goal-aware wait timeout: the effective duration plus whether the goal cap
/// actually shortened what the model asked for.
pub(crate) fn goal_capped_wait_timeout(
    timeout_ms: Option<u64>,
    goal_active: bool,
) -> (Duration, bool) {
    let timeout = capped_wait_timeout(timeout_ms, goal_wait_cap(goal_active));
    let uncapped = capped_wait_timeout(timeout_ms, max_wait_block());
    (timeout, timeout < uncapped)
}
...
impl TaskOutputTool {
    async fn run_single_task(
        ...
        // Goal mode tightens the standard block cap; upstream's wait hint is
        // then computed against whichever cap actually applies, so the
        // reported "waited" matches what the model really got.
        let wait_cap = goal_wait_cap(goal_active);
        let goal_capped = waits && wait_cap < max_wait_block();
        ...
        if let Some(snapshot) = snapshot {
            ...
            let mut result = apply_running_wait_hint(
        ...
        if let Some(backend) = backend
            && let Some(snapshot) = backend
                ...
                .await
        {
            let mut out = format_subagent_snapshot(&snapshot, wait_hint);
            if goal_capped
