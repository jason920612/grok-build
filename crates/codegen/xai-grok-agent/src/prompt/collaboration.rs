...
2. Keep it true as you go: mark the phase you actually work on `active`; when physically blocked on an external process, mark it `waiting` with note = what you wait on — a declared wait is protected (no nagging, no stuck signals) and waiting is valid work, so do not invent busywork to fill it. For a standing duty with no finish line (service upkeep, a live monitoring rhythm), mark the phase `ongoing` with note = what the duty is and checkInMinutes — the framework wires the scheduled check-in automatically, so the project goal can complete while the duty stays supervised; dissolve a duty by marking it `done` with evidence. Mark phases `done` only with evidence; completion claims are spot-checked.
