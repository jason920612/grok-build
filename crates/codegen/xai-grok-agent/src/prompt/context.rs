...
impl PromptContext {
    ...
    pub fn render_with_renderer(&self, renderer: &TemplateRenderer) -> Option<String> {
        ...
        // whole team follows it. Lives here rather than in `render` so the
        // bridge-less path gets it too.
        if let Some(collab) = render(crate::prompt::collaboration::COLLABORATION_TEMPLATE) {
