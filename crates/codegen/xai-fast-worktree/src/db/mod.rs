...
impl WorktreeDb {
    ...
    ///
    /// Discovers grok home via `$GROKTOOL_HOME`, falling back to the canonicalized
...
pub fn resolve_grok_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("GROKTOOL_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home =
        PathBuf::from(std::env::var("HOME").context("neither $GROKTOOL_HOME nor $HOME is set")?);
    // Canonicalize the home dir so worktree paths share the same physical
    // .groktool tree as trust/hooks even when it is symlinked. The dunce
    // canonicalization, the dot-dir name, and the env var must stay in sync
    // with xai_grok_config::paths (USER_DOT_DIR / HOME_ENV_VAR); this crate
    // is deliberately standalone so it cannot import them. Home resolution
    // deliberately differs ($HOME here vs std::env::home_dir()).
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".groktool"))
}

/// Serializes tests that mutate the process-global `GROKTOOL_HOME` env var so they
...
#[cfg(test)]
static GROKTOOL_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`GROKTOOL_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `GROKTOOL_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `GROKTOOL_HOME` before `_lock` releases, so the env is correct
...
impl GrokHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = GROKTOOL_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("grok-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via GROKTOOL_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until GROKTOOL_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("GROKTOOL_HOME");
        unsafe { std::env::set_var("GROKTOOL_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
    ...
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("GROKTOOL_HOME", p),
                None => std::env::remove_var("GROKTOOL_HOME"),
            }
        }
    }
}
