...
//! `$GROKTOOL_HOME` is resolved through `xai-grok-config::grok_home`.
...
/// This function caches the result in `$GROKTOOL_HOME/agent_id` so subsequent calls
...
/// Write `$GROKTOOL_HOME/agent_id` as owner-read/write only (Unix 0o600) — it is a
