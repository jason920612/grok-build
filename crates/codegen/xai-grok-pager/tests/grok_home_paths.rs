//! `GROKTOOL_HOME` override tests in an isolated binary so `grok_home()`'s
...
fn grok_home_override_path_helpers() {
    ...
    unsafe {
        std::env::set_var("GROKTOOL_HOME", &grok_home);
    }
