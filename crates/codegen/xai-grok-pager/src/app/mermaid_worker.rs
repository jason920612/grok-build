...
thread_local! {
    ...
    /// `GROKTOOL_HOME` (whose `grok_home()` value is cached first-write-wins, an
    ...
    /// `subagent::REPLAY_GROKTOOL_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `GROKTOOL_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    /// hermetic, with no process-global `GROKTOOL_HOME` mutation. The `TempDir` lives
