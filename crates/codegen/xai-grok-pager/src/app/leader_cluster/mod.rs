...
//!
//! Env sandboxing follows this crate's `serial(GROKTOOL_HOME)` idiom; note
...
//! `GROK_LEADER_SOCKET`, `GROKTOOL_HOME`) for a real agent's whole lifetime, and
...
struct PagerLeaderCluster {
    ...
    /// agent on the same GROKTOOL_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(GROKTOOL_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let env = vec![
            crate::test_util::EnvVarGuard::set("GROKTOOL_HOME", grok_home.path()),
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same GROKTOOL_HOME when the next generation's
