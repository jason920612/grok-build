...
#[cfg(test)]
thread_local! {
    static REPLAY_GROKTOOL_HOME: std::cell::RefCell<Option<std::path::PathBuf>> =
...
pub(crate) fn set_replay_grok_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_GROKTOOL_HOME.with(|h| *h.borrow_mut() = home);
}
#[cfg(test)]
fn effective_grok_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_GROKTOOL_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
    xai_grok_shell::util::grok_home::grok_home()
}
